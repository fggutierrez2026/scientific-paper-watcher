...
|---|---|---|---|---|---|---|
| [Semantic Scholar Academic Graph](https://api.semanticscholar.org/api-docs) | `GET api.semanticscholar.org/graph/v1/paper/search`; Allen Institute for AI | An API key is optional for shared access and recommended for dedicated throughput. Send it only in the case-sensitive `x-api-key` header. | Follow the API license and rate limit assigned to the key; authenticated introductory access is currently documented as 1 request/second. Handle HTTP 429, `Retry-After`, and exponential backoff. | Relevance search uses `offset`/`limit` with at most 100 records per page and 1,000 relevance-ranked results. It accepts `publicationDateOrYear`; bulk search instead uses token pagination and a different query language. | Paper ID, Corpus ID, DOI, PMID, arXiv ID, title, abstract, authors, venue, date/year, citations, fields of study and OA PDF | **ready** |
